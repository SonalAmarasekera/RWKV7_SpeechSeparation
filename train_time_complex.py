...
def main():
    ...
    complexity_log_path = os.path.join(args.save_dir, "complexity_td_domain.txt")
    complexity_meter = ComplexityMeasurement(model, device, log_file=complexity_log_path)
    complexity_meter.measure_model_complexity(input_shape=(args.batch_size, T_enc, codec_dim))
    
    # Measure CodecFormer-style inference MACs (for fair comparison)
    complexity_meter.measure_inference_macs(
        sample_rate=args.sample_rate,  # Typically 8000 for time domain
